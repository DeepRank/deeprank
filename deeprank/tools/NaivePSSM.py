...
class NaivePSSM(FeatureClass):
	...
	def __init__(self,pdbfile,pssm_path,nmask=17,nsmooth=3):

		super().__init__("Residue")

		self.pdbfile = pdbfile
		self.pssm_path = pssm_path
		self.molname = self.get_mol_name(pdbfile)
		self.nmask = nmask 
	...
	@staticmethod
	def get_mol_name(pdbname):

		molname = os.path.basename(pdbname)
		molname = os.path.splitext(molname)[0]
		return molname.split('_')[0]
	...
	def get_feature_value(self,contact_only=True):
		...
		for res,data in zip(self.res_data,self.pssm_data):
			...
			if tuple(res) in xyz_dict:
				pos = xyz_dict[tuple(res)]
				sasa = self.sasa[tuple(res)] 
				pssm_data_xyz[tuple(pos)] = [data*sasa]
			else:
				print(tuple(res), ' not found in the pdbfile')
...
if __name__ == '__main__':
	...
	pssm = NaivePSSM('1AK4_1w.pdb',path)
