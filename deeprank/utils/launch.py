...
BM4 = '/home/deep/projects/deeprank/data/HADDOCK/BM4/'
...
if __name__ == '__main__':
	...
	parser = argparse.ArgumentParser(description='launch multiple HDF5 calculations')
	parser.add_argument('-s','--status',action='store_true',help='Only list the directory')
	parser.add_argument('-d','--device', help="GPU device to use",default='1',type=str)
	parser.add_argument('-m','--mol',nargs='+',help='name of the molecule to process',default=None,type=str)
	parser.add_argument('-i','--init',help="index of the first molecule to process",default=0,type=int)
	parser.add_argument('-f','--final',help="index of the last molecule to process",default=0,type=int)
	parser.add_argument('--clean',help="Clean the datafiles",action='store_true')
	...
	size = [ "{:5.2f}".format(os.path.getsize(n+'.hdf5')/1E9) if n+'.hdf5' in hdf5 else '' for n in names ]
	...
	if args.status:
		...
		print('-'*50)
		print(': Status  --> %4.3f %%  done' %(status.count('Done')/len(status)*100))
		print(': Mem Tot --> %4.3f GB\n' %sum(list(map(lambda x: float(x),filter(lambda x: len(x)>0,size)))))
