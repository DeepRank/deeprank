...
#    modelIDs_sorted = modelIDs_sorted[::-1] #reverse the list
    ...
    xue = pd.DataFrame(list(zip(modelIDs, out)), columns = ['modelID',  'final_S'])
...
def plot_hit_rate_withHS(hdf5,HS_DIR=None, epoch=None,figname=None,inverse = False):
    ...
    color_plot = {'train':'red','valid':'blue','test':'green', 'HS-train': 'red', 'HS-valid':'blue','HS-test': 'green'}
    line_styles = {'train':'-', 'valid':'-', 'test':'-', 'HS-train':'--', 'HS-valid':'--', 'HS-test':'--'}
...
#                pdb.set_trace()
                hit_labels_deeprank = data[l]['hit'].value # np.ndarray, hit labels for each model: [0 1 0 0 1...]
                modelIDs_deeprank = sort_modelIDs_by_deeprank(list(data[l]['mol'][:,1]), data[l]['outputs']) # np.ndarry, models IDs ranked by deeprank

#                xue = pd.DataFrame(list(zip(modelIDs_deeprank, hit_labels_deeprank)), columns=['modelIDs_DR', 'hit_labels_DR'])
#                xue.to_csv('xue.tsv', sep="\t")
                [hit_labels_HS, modelIDs_woHS] = get_hit_labels_HS(haddockS, hit_labels_deeprank, modelIDs_deeprank)
                ...
                ax.plot(x,hitrate_HS,color = color_plot[legend], linestyle = line_styles[legend], label=f'{legend}'+' M=%d' %m)
#                pdb.set_trace()
...
#                hitrate_deeprank = rankingMetrics.hitrate(data[l]['hit'])
                ...
                x = np.linspace(0,100,m)
                ax.plot(x,hitrate_deeprank,c = color_plot[l],linestyle = line_styles[l], label=l+' M=%d' %m)

                #-- count num_hit
                num_hits = list(hit_labels_deeprank).count(1)
                print(f"According to 'hit' -> num of hits for {l}: {num_hits} out of {len(hit_labels_deeprank)}")
...
def get_hit_labels_HS(haddockS, hit_labels_deeprank, modelIDs_deeprank):
  ...
  #-- remove refe pdb from hit rate calcuatioin as we do not have HS for refe
    ...
    data = pd.DataFrame(list(zip(modelIDs_HS, HS, hit_labels_deeprank)), columns = ['modelID', 'HS', 'hit_labels'])
...
def read_haddockScoreFL(HS_DIR):
    ...
    stat_FLs = tqdm(stat_FLs, desc='read stat files for haddock scores', disable = False)
    for statFL in stat_FLs:
        ...
        for line in f:
            ...
            if len(headers) != len(values):
                sys.exit(f'header field number {len(headers)} is different from the value field number {len(values)}. Check the format of {statFL}')
        ...
        if not stats or not headers or not values:
            sys.exit(f"headers or values or stats not defined. Check the format of {statFL}")
        f.close()
...
def main():
    ...
    figname = sys.argv[3]
    plot_hit_rate_withHS(hdf5,HS_DIR='/home/lixue/DBs/BM5-haddock24/stats', epoch=epoch,figname=figname + '.hitrate_wHS.png',inverse = False)
