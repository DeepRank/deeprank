...
class pool(object):
	...
	def __def_str__(self,ilayer):
		return 'self.convlayer_%03d = nn.MaxPool3d((%d,%d,%d))' %(ilayer,self.kernel_size[0],self.kernel_size[1],self.kernel_size[2])
...
class modelGenerator(object):
	...
	def write(self):
		...
		f.write('class ' + self.name + '(nn.Module):\n')
		self._write_init(f)
...
if __name__== '__main__':
	...
	conv_layers = []
	conv_layers.append(cnn(output_size=4,kernel_size=2,post='relu'))
	conv_layers.append(pool(kernel_size=(2,2,2)))
	conv_layers.append(cnn(input_size=4,output_size=5,kernel_size=2,post='relu'))
	conv_layers.append(pool(kernel_size=(2,2,2)))
	...
	MG = modelGenerator(name='cnntest',fname='modeltest.py',conv_layers=conv_layers,fc_layers=fc_layers)
	MG.write()