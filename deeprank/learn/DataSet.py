...
class DataSet():
    ...
    def process_dataset(self):
        ...
        print('   Grid Size           : %d x %d x %d' %
              (self.data_shape[1], self.data_shape[2], self.data_shape[3]))
        sys.stdout.flush()
                        ...
                        if np.all(cond):
                            ...
                            if '*' in name:
                                ...
                                possible_names = list(
                                    mapped_data[feat_type].keys())
                                match_names = [
                                    n for n in possible_names if n.startswith(match)]
        ...
        if self.pair_chain_feature:
            ...
            for feat_type, feat_names in self.select_feature.items():
                ...
                if '_ind' in feat_type:
                    self.pair_indexes += [[i, i + 1]
                                          for i in range(start, start + nfeat, 2)]
                else:
                    self.pair_indexes += [[i]
                                          for i in range(start, start + nfeat)]
                start += nfeat
    ...
    def load_one_molecule(self, fname, mol=None):
        ...
        for feat_type, feat_names in self.select_feature.items():
            ...
            if 'mapped_features/' + feat_type in mol_data.keys():
                ...
                print(
                    'Feature type %s not found in file %s for molecule %s' %
                    (feat_type, fname, mol))
            ...
            for name in feat_names:
                ...
                try:
                    ...
                    print('Possible feature are : ' +
                          '\n\t'.join(list(mol_data['mapped_features/' +
                                                    feat_type].keys())))
        ...
        return np.array(feature).astype(
            ...
            if __vectorize__ or __vectorize__ == 'both':
                ...
                for chainID in [0, 1]:
                    tmp_feat_vect[chainID] = np.sum(
                        vmap(pos[chain == chainID, :], feat_value[chain == chainID]), 0)
